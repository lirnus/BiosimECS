...
namespace bs {
	...
	void checkForLoops_DFS(std::array<std::array<Adjacency, numberOfGenes>, NUM_NEURONS> fwd_adj, std::array<Connection, numberOfGenes> conn_list);

	void delete_connection(std::array<Connection, numberOfGenes> conn_list, int source, int sink);
}
...
In the next step, a selection pressure gets applied on these pixies, causing some to perish (fitness = 0) and others to reproduce, getting chosen to pass on their genes to the next generation. The next generation gets filled up with individuals from the previous one, reaching the same size, while choosing individuals with a chance proportional to their fitness. <br> 
Over many generations, we expect to see that Pixies with an appropriate behaviour survive and thrive, making up the biggest part of the population. <br>
This selection pressure could for example be to *only select Pixies on the left half of the grid* (or, more accurately, setting the fitness value of every pixie on the right half to 0). In this case, we would expect to mostly find Pixies that exhibit a behaviour that more frequently leads to them being on the left half, and indeed that is what we find:<br>
<p align="center">
...
```
    uint32
    0xf0ccac1a
    1111 0000 1100 1100 1010 1100 0001 1010
    ^         ^         ^
    source>   sink>     weight>
    uint8     uint8     uint16
```
...
<p align="center">
    <img alt="growing population" src="https://github.com/user-attachments/assets/30cc54e3-7b41-4da7-b892-66a353e764f7">
    <br>
    <em>An example of a "growing" simulation, favoring pixies that consume food (yellow triangles)</em>
</p>
